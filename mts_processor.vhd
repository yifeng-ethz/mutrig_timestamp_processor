...
-- Version : 26.3.3
-- Date    : 20260516
-- Change  : Restore hit_type1_out data to the legacy 39-bit Type1 payload and expose
--           the true 48-bit hit timestamp on a separate sideband for direct V3
--           histogram Type1 delay input. This removes the need to pack timestamp data
--           above the normal payload field.
-- =========
-- Description:	[MuTRiG Timestamp Processor] 
    ...
    -- ==================
    --		Total	39 legacy payload bits plus true hit timestamp on coe_hit_type1_ts[47:0]
...
architecture rtl of mts_processor is
    ...
    signal tcc_div_quotient_d : std_logic_vector(LPM_DIV_WIDTHN-1 downto 0);
        ...
        if (i_rst = '1') then 
            ...
            case processor_state is 
                ...
                when FLUSHING => -- drain accepted payload and wait for the explicit upstream end-of-run pulse
                    ...
                    if (run_state_cmd = RUN_PREPARE) then -- reset/re-arm even if a previous terminate did not drain
                        processor_state		<= RESET;
                        reset_flow			<= SCLR;
                        ...
                        processor_state		<= RUNNING;
                        reset_flow			<= DONE;
                    elsif (run_state_cmd = IDLE) then -- standard sequence
                        processor_state		<= IDLE;
                        reset_flow			<= DONE;
                    end if;
                when others => 
    ...
    begin
        if (rising_edge(i_clk)) then
            if (DEBUG_TRACE_REPORTS and DEBUG /= 0 and i_rst = '0' and csr.soft_reset = '0') then
                ...
                if (hit_totcalc.valid = '1') then
                    report "MTS_STAGE[" & BANK & "] hit_totcalc ch="
                        & integer'image(to_integer(unsigned(hit_totcalc.channel)))
                        severity note;
                end if;
                if (hit_div(LPM_DIV_PIPELINE).valid = '1') then
